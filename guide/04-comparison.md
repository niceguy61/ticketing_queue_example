...
> 로컬 개발 시에는 Docker로 Redis, RabbitMQ를 사용하고, 프로덕션에서는 관리형 서비스로 전환합니다.
...
| 항목 | ElastiCache 단독 | ElastiCache + Amazon MQ | ElastiCache + SQS |
|------|------------------|-------------------------|-------------------|
| 복잡도 | ⭐ | ⭐⭐⭐ | ⭐⭐ |
| 신뢰성 | ⭐⭐⭐ | ⭐⭐⭐⭐ | ⭐⭐⭐⭐⭐ |
| 확장성 | ⭐⭐⭐ | ⭐⭐⭐ | ⭐⭐⭐⭐⭐ |
...
| 기능 | ElastiCache (Redis) | Amazon MQ | SQS |
|------|---------------------|-----------|-----|
| 실시간 위치 조회 | ✅ | ❌ | ❌ |
| 메시지 영속성 | ✅ (AOF/RDB) | ✅ | ✅ |
| Dead Letter Queue | ❌ | ✅ | ✅ |
...
flowchart TD
    ...
    Q3 -->|Yes| Q4{서버리스?}
    Q3 -->|No| AMQ[ElastiCache + Amazon MQ]
    
    Q4 -->|Yes| SQS[ElastiCache + SQS + Lambda]
    Q4 -->|No| Q5{복잡한 라우팅?}
    
    Q5 -->|Yes| AMQ
    Q5 -->|No| SQS2[ElastiCache + SQS]
```
...
| 장애 유형 | ElastiCache 단독 | ElastiCache + Amazon MQ | ElastiCache + SQS |
|-----------|------------------|-------------------------|-------------------|
| 메모리 고갈 | 🔴 치명적 | 🟡 부분 영향 | 🟢 SQS 무관 |
| 서버 다운 | � 자동 Failover | 🟡 부분 복구 | 🟢 자동 복구 |
| 데이터 유실 | � 중간 (AOF 백업) | 🟡 중간 | 🟢 낮음 |
| 실수 삭제 | � 스냅샷 복구 | 🟡 정의 백업 | 🟢 자동 보존 |
| 네트워크 단절 | � Multi-AZ 복구 | 🟡 파티션 위험 | 🟢 재시도 |
| 처리량 초과 | 🟡 느려짐 | 🟡 블로킹 | 🟡 쓰로틀링 |

### 복구 난이도

| 상황 | ElastiCache | Amazon MQ | SQS |
|------|-------------|-----------|-----|
| 서비스 재시작 | 자동 | 중간 | 불필요 |
| 데이터 복구 | 스냅샷 복원 | 중간 | 쉬움 |
| 설정 복원 | 파라미터 그룹 | 정의 파일 | 자동 |
| 장애 원인 분석 | CloudWatch | CloudWatch | CloudWatch |
...
| 장애 | ElastiCache 단독 | Amazon MQ | SQS |
|------|------------------|-----------|-----|
| 노드 다운 | 자동 Failover (초~분) | Failover (분) | 자동 (초) |
| 메모리 부족 | 알람 + 스케일업 (분) | 자동 블로킹 | 해당 없음 |
| 디스크 부족 | 해당 없음 | 자동 블로킹 | 해당 없음 |
| 설정 오류 | 파라미터 그룹 수정 (분) | 재설정 (분) | 콘솔 수정 (초) |
